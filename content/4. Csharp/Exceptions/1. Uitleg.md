---
title: Uitleg Exceptions
draft: true
tags: 
taxonomie:
  - ib-19.2.Exceptions.2.OI
  - ib-19.2.Exceptions.3.OI
---
...
## Hoe pas je het opvangen van Exceptions toe?
...
### Soorten exceptions
Er zijn verschillende soorten exceptions, zoals `DivideByZeroException` of `FileNotFoundException`, en het is handig om specifieke exceptions af te handelen, zodat je gericht kunt reageren op verschillende fouten. Als vangnet kun je een algemene `catch (Exception ex)` gebruiken, die alle uitzonderingen opvangt, maar het is beter om dit alleen als laatste optie te gebruiken.

### Finally
De `finally`-blok wordt altijd uitgevoerd, ongeacht of er een exception is opgetreden in het `try`-blok. Het is ideaal voor opruimwerkzaamheden, zoals het sluiten van bestanden of het vrijgeven van bronnen. Code in de `finally`-blok wordt altijd uitgevoerd, zelfs als er een exception was of niet. Dit zorgt ervoor dat belangrijke acties niet worden overgeslagen.
...
> Volgende stap: [[2. Oefeningen|Oefeningen]]
