...
Je werkt aan een applicatie waarbij een klasse zowel verantwoordelijk is voor het ophalen van gegevens, het berekenen van een resultaat, en het tonen van een bericht. Dit is niet ideaal omdat de klasse meerdere verantwoordelijkheden heeft, wat volgens het Single Responsibility Principle (SRP) niet wenselijk is. De code in de klasse compileert wel, omdat er is gewerkt met tuple (een klein pakketje van meerdere waarden bij elkaar, zonder dat je er een aparte klasse of record voor hoeft te maken), maar we willen deze klasse refactoren naar kleinere, modulaire klassen waarbij elke klasse één verantwoordelijkheid heeft.
...
{
    private List<(int Id, decimal Amount)> _orders;

    public OrderService()
    {
        _orders = new List<(int Id, decimal Amount)>
        {
            (Id: 1, Amount: 100m),
            (Id: 2, Amount: 200m),
            (Id: 3, Amount: 150m)
    ...
    public List<(int Id, decimal Amount)> GetOrders()
    {
        return _orders;
    }
...
{
    private List<(int Id, decimal Amount)> _orders;
    private List<string> _dbContext;
    private List<string> _emailService;
    private List<string> _notificationService;
    
    public OrderService()
    {
        _orders = new List<(int Id, decimal Amount)>();
        _dbContext = new List<string>();
        _emailService = new List<string>();
        _notificationService = new List<string>();
    }

    public void PlaceOrder(List<(int Id, decimal Amount)> order)
    {
        foreach (var item in order)
        {
            // Sla bestelling op in de database
            string dbMessage = $"Bestelling {item.Id} opgeslagen.";
            _dbContext.Add(dbMessage);

            // Stuur een bevestigingsmail naar de klant
            string emailMessage = $"Bevestigingsmail verzonden voor bestelling {item.Id}.";
            _emailService.Add(emailMessage);

            // Verstuur een notificatie naar de klant
            string notificationMessage = $"Notificatie verstuurd voor bestelling {item.Id}.";
            _notificationService.Add(notificationMessage);
        }
    }
}
``` 
...
{
  private List<(int Id, decimal Amount)> _orders;
  private List<(int Id, decimal Amount, DateTime date)> _invoices;
    ...
    public OrderProcessor()
    {
        ...
        _invoices = new List<(int Id, decimal Amount, DateTime Date)>();
    }

public void ProcessOrder((int Id, decimal Amount) order)
    {
        // Bestelling verwerken
    ...
    private (int Id, decimal Amount, DateTime Date) GenerateInvoice((int Id, decimal Amount) order)
    {
            var invoice = (Id: order.Id, Amount: order.Amount, Date: DateTime.Now);
            return invoice;
    }

    private void SaveInvoice((int Id, decimal Amount, DateTime Date) invoice)
    {
        _invoices.Add(invoice);
        Console.WriteLine($"Saving invoice for order {invoice.Id}");
    }

    private void SendConfirmationEmail((int Id, decimal Amount) order)
    {
