...
## Hoe zit `.gitignore` in elkaar?
Platforms voor versiebeheer, zoals Github en Gitlab, werken met speciale bestanden waarmee functionaliteit geconfigureerd kan worden. Eén daarvan `.gitignore`. Voordat wijzigingen worden toegevoegd via git wordt het bestand `.gitignore` geraadpleegd welke mappen en bestanden moeten worden uitgesloten.
